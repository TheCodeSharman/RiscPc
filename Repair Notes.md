...
- **PCOMP** (phase-comparator out): at a good probe point shows a clean **periodic sawtooth** = charge pump actively pumping. (An earlier "solid 5 V" reading was a bad probe point — discount it.)
  > **[Superseded — see Jun 16 +12V analysis]** Re-interpretation: PCOMP is a *tristate* output (HIGH = "speed up", LOW = "slow down", HI-Z = "in lock, no correction"). A healthy locked loop shows mostly HI-Z with small symmetric HIGH/LOW pulses around reference edges, **not** a sustained sawtooth. What was observed here was PCOMP **railed mostly HIGH with occasional brief glitches** — the unambiguous signature of a loop pegged at one rail trying continuously to speed the VCO up and failing. Misread as healthy charge-pump ripple, this directly led to "hardware exonerated" and cost us subsequent sessions chasing CMOS/config theories. The actual fault: missing +12V starved Q2's PNP headroom, so Vcc_04 couldn't rise to reach 100.8 MHz, and the charge pump fired HIGH continuously trying to push it there.
...
> **[Superseded — see Jun 16 +12V analysis]** "Hardware fully exonerated" was wrong. The VCO loop was *not* healthy — it was railed at its low end because missing +12V destroyed Q2's PNP compliance. The "rock-steady 14 MHz" was the propagation-delay floor of the inverter at starved Vcc_04, not a stable lock to a commanded value. The "clean sawtooth" on PCOMP was actually PCOMP pegged HIGH with brief glitches — the loop saying "go faster!" every cycle and getting no response. Every observation in this conclusion fit the data we had, but the model behind it (a healthy loop locked to a low command) was wrong: it was an unhealthy loop unable to reach a high command. The CMOS-config hypothesis we pursued from here was a misdirection. See Jun 16 / Jun 17 for the corrected picture.
...
**Scope probe points (for next time):** VCO output = **IC32 pin 6**; PCOMP at the loop-filter node (not the railed-supply point); Vcc_04 = the 74AC04 supply rail. Reference = 24 MHz refclk (X2/IC4), rock-solid. **What to look for at PCOMP**: in a healthy locked loop, mostly HI-Z with small symmetric HIGH/LOW pulses around reference edges. PCOMP railed continuously HIGH (or LOW) with only brief opposite-polarity glitches = loop pegged trying to push frequency one direction and unable to — fault is downstream in the analog control chain (Q2/Q3/Vcc_04), not in the digital reference.
...
- **VCO / PLL loop** — *believed* healthy at session 4 (VCO output locks rock-steady, PCOMP pumps a clean sawtooth, Vcc_04 shows what looked like a textbook acquisition transient) — **but this reading was wrong**, see superseded stamp on the session 4 conclusion. The "sawtooth" was PCOMP railed HIGH with brief glitches, and the "lock" was the inverter sitting on its propagation-delay floor at a starved Vcc_04. Actual fault was the missing +12V rail (Jun 16 / Jun 17).
...
> **[Superseded — see Jun 16 +12V analysis]** Hypothesis A turned out to be correct, but for a reason we hadn't identified at this point: the analog-loop fault was missing +12V on the VCO bias network, not a damaged component. The "rock-steady, clean PCOMP sawtooth, textbook acquisition" *were* session 4 observations but they were misreads — see the superseded stamp on the session 4 conclusion. PCOMP railed HIGH is a starvation signature, not a healthy lock. Once that's flipped, Hypothesis A fits session 4 perfectly, and the bus-corruption Hypothesis B becomes unnecessary.
