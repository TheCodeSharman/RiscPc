...
- **`upstream`** — RPCEmu upstream verbatim **plus git-conversion infra**.
  ...
  `v0.9.6`, ...); sitting on top of that tag are a few commits that exist
  *only* because we converted hg→git and aren't meaningful upstream
  changes: `.gitignore`, untracking the runtime `cmos.ram`/`rpc.cfg`
  (RPCEmu regenerates both with defaults), and `tools/reintegrate.sh`.
  Keeping them here keeps them **out** of the feature-vs-upstream patches
  we email to marutan.  Only changes on a new import (or an infra tweak).
- **`integration`** — the **derived integration branch** (the repo's
  default branch; formerly `main`).  It is `upstream` + one **squashed**
  commit per feature branch, regenerated by `tools/reintegrate.sh` —
  never hand-edited.  This is what `setup-rpcemu.sh` clones and what
...
- **`feature/vram-honesty`** and other `feature/*` branches —
  **long-lived patch branches** off `upstream`.  Each carries one
  ...
  to marutan when ready); `integration` picks them up via re-integration.

Why this split: the `upstream` / `feature/*` separation keeps each patch
isolated for clean upstream-submission diffs (`git diff upstream
feature/X`); the derived `integration` branch gives consumers
(`setup-rpcemu.sh`) a single predictable target without having to know
...
3. Commit as `Import RPCEmu x.y.z`, tag as `vx.y.z`, then re-apply the
   git-conversion infra commits on top (they live on `upstream`)
